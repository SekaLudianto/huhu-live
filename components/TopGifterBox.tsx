        ...
        {topGifters.length > 0 ? (
          topGifters.slice(0, 100).map((entry, index) => (
            <div key={entry.user.uniqueId} className="bg-white/50 dark:bg-gray-800/50 p-2 rounded-lg flex items-center text-sm">
              <span className={`text-2xl font-bold w-8 text-center ${titleColors[index] || 'text-gray-700 dark:text-gray-300'}`}>{index + 1}</span>
              <img className="w-10 h-10 rounded-full mx-3 border-2 border-gray-400 dark:border-gray-600" src={entry.user.profilePictureUrl} alt={entry.user.nickname} />
              ...
              <div className="text-right pl-2 flex items-center gap-1">
                <span className="font-bold text-lg text-yellow-600 dark:text-yellow-400">{formatDiamonds(entry.totalDiamonds)}</span>
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4 text-yellow-500 dark:text-yellow-400">
