...
const GiftBox: React.FC<GiftBoxProps> = ({ latestGift }) => {
    ...
    return (
        <div className="bg-gray-300/50 dark:bg-gray-700/50 rounded-lg p-4 flex flex-col h-full">
            <h3 className="text-lg font-bold text-center text-gray-900 dark:text-white mb-3">Hadiah</h3>
            <div className="flex-grow overflow-y-auto pr-2 space-y-2" ref={containerRef}>
                {gifts.map((gift, index) => (
                    <div key={`${gift.giftId}-${index}`} className="bg-white/50 dark:bg-gray-800/50 p-2 rounded-lg flex items-start text-sm text-gray-800 dark:text-gray-200">
                        ...
                        <div className="flex-1">
                            ...
                            <div className="flex items-center mt-1 bg-gray-200/50 dark:bg-gray-700/50 p-1 rounded">
                                <img className="w-10 h-10 mr-2" src={gift.giftPictureUrl} alt={gift.giftName} />
                                <div>
                                    <div className="font-semibold text-gray-900 dark:text-white">{gift.giftName}</div>
                                    <div>
                                        <span className="font-bold text-yellow-600 dark:text-yellow-400">{(gift.diamondCount * gift.repeatCount).toLocaleString()}</span> Diamonds
                                        {gift.repeatCount > 1 && <span className="ml-2 text-gray-500 dark:text-gray-400">x{gift.repeatCount}</span>}
                                    </div>
                                </div>
                            </div>
                        </div>
