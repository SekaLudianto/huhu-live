...
const Modal: React.FC<ModalProps> = ({ isOpen, onClose, title, children, isActionDisabled = false, actionDelay = 0 }) => {
    ...
    const isButtonDisabled = isActionDisabled || countdown > 0;
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center p-4 z-50 transition-opacity duration-300 ease-in-out" style={{opacity: isOpen ? 1 : 0}}>
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-md transform transition-all duration-300 ease-in-out" style={{transform: isOpen ? 'scale(1)' : 'scale(0.95)'}}>
                <div className="p-6">
                ...
                <div className="bg-gray-100 dark:bg-gray-900/50 px-6 py-3 text-right rounded-b-lg">
                    <button 
                        onClick={onClose} 
